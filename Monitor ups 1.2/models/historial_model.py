...
def nombre_archivo_semana() -> str:
    hoy = datetime.now()
    anio, semana, _ = hoy.isocalendar()
    return os.path.join(HISTORIAL_DIR, f'historial_{anio}-S{semana:02d}.xlsx')

def guardar_historial(datos: list[dict]) -> None:
    if not EXCEL_OK:
        return
    os.makedirs(HISTORIAL_DIR, exist_ok=True)
    archivo = nombre_archivo_semana()
...
def leer_historial(archivo: str) -> dict:
    ...
    return {'cabeceras': cab, 'filas': datos[-500:]}