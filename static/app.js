...
const UMBRAL_BATERIA_ALERTA = 40;
...
function mostrarVista(v, btn) {
  ...
  if (v === 'historial') iniciarHistorial();
...
function nivelCard(u) {
  ...
  if (['Falla', 'Apagando', 'Sin respuesta'].includes(u.estado)) return 'rojo';
  if (['En bateria', 'Bypass'].includes(u.estado))               return 'naranja';
  if (u.voltaje_entrada !== null && u.voltaje_entrada < UMBRAL_VOLTAJE_MIN)    return 'naranja';
  if (u.bateria_pct     !== null && u.bateria_pct     < UMBRAL_BATERIA_ALERTA) return 'naranja';
...
function badgeInfo(nivel, u) {
  ...
  if (nivel === 'rojo') {
    ...
    return { cls: 'badge-rojo', txt: u.estado.toUpperCase() };
  }
  if (nivel === 'naranja') {
    if (u.estado === 'Bypass')     return { cls: 'badge-naranja', txt: 'BYPASS' };
    if (u.estado === 'En bateria') return { cls: 'badge-naranja', txt: 'EN BATERIA' };
    if (u.voltaje_entrada !== null && u.voltaje_entrada < UMBRAL_VOLTAJE_MIN)
      return { cls: 'badge-naranja', txt: 'V.ENTRADA BAJO' };
    if (u.bateria_pct !== null && u.bateria_pct < UMBRAL_BATERIA_ALERTA)
      return { cls: 'badge-naranja', txt: 'BAT. BAJA' };
    return { cls: 'badge-naranja', txt: 'CARGA ALTA' };
  }
  return { cls: 'badge-ok', txt: 'EN LINEA' };
}

function colorBat(v) {
  return v === null ? '#555' : v < UMBRAL_BATERIA_ALERTA ? '#ff9800' : '#4caf50';
}
...
// ── HISTORIAL ─────────────────────────────────────────────
let histArchivos    = [];
let histDiaActual   = null;
let histDatosActual = null;
let histUPSFiltro   = 'todos';
let calMes          = new Date();
let histCargado     = false;
let histPagina      = 0;
const HIST_FILAS_PAG = 500;

async function iniciarHistorial() {
  renderCalendario(); // mostrar estructura de inmediato
  if (histCargado) return;
  try {
    histArchivos = await (await fetch('/api/historial/lista')).json();
  } catch(e) {
    histArchivos = [];
  }
  histCargado = true;
  const fechas = histArchivos
    .map(f => f.replace('historial_', '').replace('.xlsx', ''))
    .filter(f => /^\d{4}-\d{2}-\d{2}$/.test(f))
    .sort();
  if (fechas.length) {
    const [a, m] = fechas[fechas.length - 1].split('-');
    calMes = new Date(+a, +m - 1, 1);
  }
  renderCalendario(); // re-renderizar con días resaltados
}
...
function renderCalendario() {
  ...
  document.getElementById('cal-mes-titulo').textContent = MESES[mes] + ' ' + anio;
  ...
  let html = ['Lu','Ma','Mi','Ju','Vi','Sa','Do']
    .map(d => `<div class="cal-dia-hdr">${d}</div>`).join('');

  for (let i = 0; i < inicio; i++) html += '<div class="cal-dia"></div>';

  for (let d = 1; d <= ultimo; d++) {
    const f   = `${anio}-${String(mes+1).padStart(2,'0')}-${String(d).padStart(2,'0')}`;
    const ok  = conDatos.has(f);
    const cls = ['cal-dia', ok ? 'tiene-datos' : '', f === hoyStr ? 'hoy' : '', f === histDiaActual ? 'seleccionado' : ''].filter(Boolean).join(' ');
    html += `<div class="${cls}" ${ok ? `onclick="seleccionarDia('${f}')"` : ''}>${d}</div>`;
  }
  document.getElementById('hist-calendario').innerHTML = html;
}
...
async function seleccionarDia(fecha) {
  ...
  const archivo = `historial_${fecha}.xlsx`;
  const wrap    = document.getElementById('hist-tabla-wrap');
  const filtro  = document.getElementById('hist-ups-filtro');
  ...
  if (!d.filas || !d.filas.length) {
    wrap.innerHTML = '<p class="hist-vacio">Sin datos para esta fecha.</p>';
    document.getElementById('btn-descargar').classList.add('hidden');
    return;
  }

  const nombres = [...new Set(d.filas.map(f => f[1]))].sort();
  let pills = `<button class="ups-pill activa" data-nombre="todos" onclick="filtrarUPS(this)">Todas (${nombres.length})</button>`;
  nombres.forEach(n => {
    pills += `<button class="ups-pill" data-nombre="${n}" onclick="filtrarUPS(this)">${n}</button>`;
  });
  filtro.innerHTML = pills;
...
function filtrarUPS(btn) {
  histUPSFiltro = btn.dataset.nombre;
  histPagina    = 0;
  document.querySelectorAll('.ups-pill').forEach(p => p.classList.remove('activa'));
  btn.classList.add('activa');
  renderTablaHistorial();
}

function irPaginaHist(n) {
  histPagina = n;
  renderTablaHistorial();
  document.getElementById('hist-tabla-wrap').scrollTop = 0;
}

function renderTablaHistorial() {
  const d     = histDatosActual;
  const todas = histUPSFiltro === 'todos'
    ? d.filas
    : d.filas.filter(f => f[1] === histUPSFiltro);

  const total    = todas.length;
  const paginas  = Math.max(1, Math.ceil(total / HIST_FILAS_PAG));
  if (histPagina >= paginas) histPagina = paginas - 1;
  const desde    = histPagina * HIST_FILAS_PAG;
  const filas    = todas.slice(desde, desde + HIST_FILAS_PAG);
  ...
  filas.forEach(f => {
    const e   = f[6] || '';
    const cls = ['Falla','Sin respuesta'].includes(e) ? 'tr-rojo'
              : ['En bateria','Bypass'].includes(e)   ? 'tr-naranja' : '';
    html += `<tr class="${cls}">`;
  ...
  // Paginación debajo del wrap
  let pag = `<div class="hist-paginacion">
    <span class="pag-total">${total.toLocaleString()} registros</span>`;
  if (paginas > 1) {
    pag += `<button onclick="irPaginaHist(${histPagina - 1})" ${histPagina === 0 ? 'disabled' : ''}>&#8249; Anterior</button>
    <span class="pag-info">Pág ${histPagina + 1} de ${paginas}</span>
    <button onclick="irPaginaHist(${histPagina + 1})" ${histPagina >= paginas - 1 ? 'disabled' : ''}>Siguiente &#8250;</button>`;
  }
  pag += '</div>';

  // Insertar paginación fuera del wrap (scroll no la corta)
  const existing = document.getElementById('hist-paginacion-ext');
  if (existing) existing.remove();
  const div = document.createElement('div');
  div.id = 'hist-paginacion-ext';
  div.innerHTML = pag;
